...
\usepackage{dsfont,tikz-cd,minitoc,pifont,tabu,enumerate}
...
\item If furthermore all negative terms in the Fourier expansions of each
  ...
  \item If the coefficient $a_0$ in all Fourier expansions from the previous
    items are also 0, then $f$ is called a \defin{cusp form of weight $k$ for
      $\Gamma'$}. The space of such functions is written $S_k(\Gamma').$
  \end{enumerate}
\end{defn}
...
Theta function as a model and define the \defin{automorphy factor}
...
The positive integer $s_r$ is called the \defin{Selmer rank of $E$}, and we have
$s_r \geq r.$
