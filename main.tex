...
\usepackage{dsfont,tikz-cd,minitoc,pifont,tabu}
...
\begin{defn}
  Let $V$ be a projective variety over $\FF_p$.
...
\begin{example}
  If $V = \mathbb{H}^n \subset \mathbb{P}^n$ is the hyperplane at infinity
...
We will consider the homomorphism
...
\begin{proof}
  ...
  Let $(u,v) = P + (n,0)$. In order to get a contradiction, assume
  $(u,v) \not\in E(\QQ_2)_0$ so $\pi(u,v) = (1,0)$.
  Then $u$ must be a unit. Let $\lambda$ be the gradient of the
  line between $(x,y), (n,0)$ and $(u,v)$.
  ...
  But this is a contradiction since if
  $$x+n = 2^n\alpha, \text{ for } n \in \NN, \alpha \in \ZZ_2^{\times}$$ then
  $$x-n = 2^n\alpha - 2n
  = 2^{n-1}(2\alpha-n).$$
  We have $2\alpha-n \not\equiv 0 \, (2)$, so $x+n$ and $x-n$ must have different 2-adic
  valuations, and hence their quotient can't be a unit.
...
\begin{cor}
  ...
  \[\prod\limits_p c_p = 2 \cdot 4^{\omega(n)}, \]
...
\[L(E_n,1) = \frac{1}{2}\Omega_{E_n}a_n^2\]
...
\begin{equation*}
  \begin{split}
    ...
    \prod\limits_p c_p \\
    &= \frac{\# \Sha(E_n) R_{E_n} \Omega_{E_n}}{8}\cdot 4^{\omega(n)}.
  \end{split}
\end{equation*}

Putting both expressions for $L(E_n, 1)$ together, the BSD conjecture predicts that 
\begin{equation} \label{mainpred} 
  a_n^2 = \#\Sha(E_p) \cdot R_{E_n}\cdot 4^{\omega(n)-1 } \cdot 0^r.
\end{equation}

The calculations in this project have allowed us to prove this holds modulo 2.

\begin{thm} We have
  \begin{equation} \label{mainthm}
    a_n^2 \equiv \#\Sha(E_n) \cdot R_{E_n} \cdot 4^{\omega(n)-1} \cdot 0^r \mod{2}.
  \end{equation}
\end{thm}

\begin{proof}
  There are three possibilities to consider, depending on the value of $\omega(n)$.

  \textbf{Case 1:} If $\omega(n) > 2$, where $n$ has more than 2 prime 
  factors, both sides
    of \autoref{mainpred} are 0 mod 16, so the theorem is trivially satisfied.

  \textbf{Case 2:} If $\omega(n) = 1$, $n=p$ is an odd prime. We want
    to show
    \[a_p^2 \equiv \# \Sha(E_n) \cdot R_{E_n} \cdot 0^r \mod {2}.\]
  ...
  \end{equation*}
  \begin{itemize}
  \item If $p \equiv 3 \, (8)$, we see that $a_p^2 \equiv 4 \,
    (16).$ In particular,
    it is not 0, so \autoref{mainpred} predicts we should have $r = 0$. 
    But that is precisely what the Selmer rank calculation tells us, since $s_r =
    0$ and $0 \leq r \leq s_r.$

  \item If $p \not\equiv 3 \, (8),$ the coefficient $a_p$ is 0 modulo 4. Thus,
    the BSD conjecture predicts that
    \[\# \Sha(E_p) \cdot R_{E_n} \cdot 0^r \equiv 0 \mod{2}.\]

    If the rank is nonzero, this is trivially satisfied, since the left hand
    side is then precisely 0.
    If $r = 0$, $R_{E_p} = 1$, so we expect 
    \[\# \Sha(E_p) \equiv 0 \mod{2}.\]
    In this case, $s_r > 0$, so if $r=0$, at least one of the equations 
    \ref{eq:isocurve} have no rational solutions even though it has real
    nontrivial solutions and solutions in 
    $\QQ_l$ for all primes $l$. Thus in all cases 
