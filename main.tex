...
if $x, y, z \neq 0$, there are in fact 8 distinct solutions given by
...
to $A_n$ modulo 4.
\begin{lemma}
  The sum $A_n$ is congruent to 0 modulo 4.
\end{lemma}
\begin{proof}
...
\begin{proof} Proceeding case by case:
  ...
  \item If $d_1 = 2,$ we're trying to solve
    \[N^2 = 2M^4 + 2p^2.\]
      ...
      In particular,
      \[N^2 \equiv 2M^4 \mod{p^2},\]
      or
      \[2 \equiv \legendre{N}{M^2}^2 \mod{p^2}.\]

      This has a nontrivial solution if and only if $\legendre{2}{p} = 1$ i.e.
      if $p \equiv \pm 1 \, (8).$

  \item If $d_1 = p,$ we're trying to solve
    \[N^2 = pM^4 + 4pe^4.\]

    We have $N^2 \equiv 0 \, (p)$, so we can write $N = pN'$ and
    \[pN'^2 = M^4 + 4e^4.\]

    Since $M, e$ cannot be divisible by $p$, we must have
    $$-4 \equiv \left(\frac{M}{e}\right)^4 \mod{p}.$$

    If $p \equiv 3 \, (4)$, the values taken by $x^4$ are the same as those
    taken by $x^2$, so the equation is equivalent to  
    $$-4 \equiv X^2 \, (p).$$
    This is a contradiction, since $\legendre{-1}{p} = -1$.

    If $p \equiv 1 \, (4)$ there are 2 cases to consider:

    If $p \equiv 1 \, (8),$ 1 is a 4th power and $ (\frac{2}{p}) = 1$,
    so $$-4 = (\sqrt{2} \cdot \sqrt{-1})^4.$$

    Hence, we have the solution $(0,(-4)^{1/4},1)$.

    If $p \equiv 5 \, (8)$, let $\chi$ be
    a primitive quartic character.
    Then $$\chi(-4) = \chi(-1)\chi(2)^2 = \chi(-1) \legendre{2}{p} = (-1) \cdot
    (-1) = 1,$$ since -1 is not a fourth power.
    Again we have the solution $(0,(4)^{1/4},1).$

    Thus, we have a solution if and only if $p \equiv 1 \, (4)$.

  \item If $d_1 = 2p$, we're trying to solve
    \[N^2 = 2pM^4 + 2pe^4.\]
    Again we may set $N = pN'$ so our equation reduces to
    $$pN'^2 = 2M^4 + 2e^4.$$
    By the same argument as the previous case, we must have
    \[-1 \equiv \legendre{M}{e}^4 \mod{p},\]
    and 1 is a $p$-adic 4th power if and only if $p \equiv 1 \, (8)$.
    In this case a solution is given by $(0, (-1)^{1/4}, 0)$.

  \item If $d_1 = -1,$ we're trying to solve
    \[N^2 = -M^4 + 4p^2e^4.\]
    In particular,
    $$N^2 \equiv -M^4 \mod{p^2},$$ so if $M \not\equiv 0 \, (p^2),
    -1 \equiv (\frac{N}{M^2})^2 \, (p)$, which has a solution if and only if
    $p \equiv 1 \, (4)$ given by $(\sqrt{-1},1,0)$.

    If $M \equiv 0 \, (p^2)$, $N$ must also be
    divisble by $p$, and we don't have a nontrivial solution in $\ZZ/p\ZZ$.

  \item If $d_1 = -2$, we're trying to solve
    \[N^2 = -2M^4 + 2p^2e^4,\]
    a similar argument shows we must have $\legendre{-2}{p}=1$,
    so $p$ must be either 1 or 3 modulo 4. A solution is then given by
    $(\sqrt{-2},1,0).$
  ...
  \item If $d_1 = -p$, we're trying to solve
    ...
    Again we may set $N = pN'$ so our equation reduces to
    $$pN'^2 = -M^4 + -4e^4.$$
    Imitating the proof for the case $d_1 = p$, we have a soltion if
    and only if -4 is a 4th power, so we must have $p \equiv 1 \, (4)$.
    It is given  by $(0,(-4)^{1/4},1).$

  \item If $d_2 = -2p$, we're trying to solve
    \[N^2 = -2pM^4 + -2pe^4.\]
    Again we may set $N = pN'$ so our equation reduces to
    $$pN'^2 = -2M^4 + -2e^4.$$
    Imitaing the proof for the case $d_1 = 2p$ we have a solution
    if and only if $p \equiv 1 \, (8)$. It is given by $(0,(-1)^{1/4},1).$
  \end{itemize} \qedhere
\end{proof}
...
Assuming the BSD conjecture (\autoref{eq:bsd}), we have
\begin{equation*}
  L(E_p,1) = \frac{\# \Sha(E) \cdot \Omega_E \cdot 8}{16} = \frac{\#\Sha(E)\cdot\Omega_E}{2}
\end{equation*}
...
We now use our knowledge of $\Sel(E)$ to predict when the value of
\[0^r \cdot \Sha(E_p)\]
is even or odd.
